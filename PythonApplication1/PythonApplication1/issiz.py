...
class Issiz(Insan):
    ...
    def set_tecrube(self, statu, deger):
        if statu in self._tecrube:
            self._tecrube[statu].append(deger)
        else:
            print("Hatali giris yaptiniz")

    def __str__(self):
        tecrube_str = f"Maviyaka Tecrube: {self._tecrube['maviyaka']}, Beyazyaka Tecrube: {self._tecrube['beyazyaka']} , Yonetici Tecrube : {self._tecrube['yonetici']}"
        return f"Ad: {self.get_ad()}, Soyad: {self.get_soyad()}, Yas: {self.get_yas()}, Cinsiyet: {self.get_cinsiyet()}, Uyruk: {self.get_uyruk()}, Tecrube: {tecrube_str}"
